...
Кастомные фильтры добавляются с помощью свойства `customFilters`.

`filterer` - функция фильтрации

`filterComponent` - кастомный компонент фильтра

`filterComponentProps` - пропсы для кастомного компонента

`initialValue` - предустановленное значение фильтра

```tsx
{
  age: {
    filterComponent: TableCustomFilterRange,
    initialValue: {min: 18, max: 100}
    filterer: (cellValue, filterValue) => {
      ...
      return Number(cellValue) <= Number(maxValue) && Number(cellValue) >= Number(minValue);
    }
  },
  name: {
    ...
    filterComponentProps: {
      withSearch: true,
      items: [
          {name: 'Антон', 'value': 'Антон'},
          {name: 'Василиса', 'value': 'Василиса'},
          {name: 'Дмитрий', 'value': 'Дмитрий'}
        ]
    },
    filterer: (cellValue, filterValues: Array<{name: string, value: string}>) => {
  ...
  title: {
    ...
    filterComponentProps: {
      items: [
          {name: 'Бурильщики', 'value': 'drillers'},
          {name: 'Геологоразведчики', 'value': 'scouts'},
        ]
    },
    filterer: (cellValue, filterValue: {name: string, value: string}) => {
      if (filterValue.value === 'drillers') {
        return cellValue === 'Бурильщик'
      }

      if (filterValue.value === 'scouts') {
        return cellValue === 'Геологоразведчик'
      }
...
Событие изменения значений кастомных фильтров можно обработать с помощью колбэка в свойстве `onCustomFiltersUpdate`
...
| ---------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| onConfirm              | (filter: { value: CustomFilterValue; isActive: boolean }) => void | Функция для сохранения обновленного значения фильтра и статуса активности фильтра |
| savedCustomFilterValue | CustomFilterValue                                                 | Значение сохраненного значения фильтра                                            |

Имеется ряд предустановленных кастомных компонентов фильтров:

| Компонент                      | Дополнительные пропсы                                               | Описание                           |
| ------------------------------ | ------------------------------------------------------------------- | ---------------------------------- |
| TableCustomFilterRange         | -                                                                   | Фильтрация по значениям "от-до"    |
| TableCustomFilterRangeGroup    | { items: {name: string, value: any} }                               | Фильтрация по одному из значенений |
| TableCustomFilterCheckboxGroup | { items: Array<{ name: string, value: any }>; withSearch: boolean } | Фильтрация по нескольким значениям |

В качестве основы для кастомного компонента рекомендуется использовать компонент `TableCustomFilterContainer`, представляющий из себя блок с кнопками подтверждения и сброса фильтра.
Он принимает в себя следующие пропсы:

| Свойство            | Тип        | Описание                                   |
| ------------------- | ---------- | ------------------------------------------ |
| onReset             | () => void | Обработчик нажатия на кнопку подтверждения |
| onConfirm           | () => void | Обработчик нажатия на кнопку отмены        |
| title               | string     | Заголовок окна                             |
| confirmButtonLabel  | string     | Текст на кнопке подтверждения              |
| resetButtonLabel    | string     | Текст на кнопку сброса                     |
| resetButtonDisabled | boolean    | Отключена ли кнопка сброса                 |
...
type CustomFilters<T extends TableRow> = { [key in RowField<T>]?: CustomFilter<T> };
...
type CustomSavedFilters<T extends TableRow> = {
...
| [`customFilters`](#кастомные-фильтры)               | `CustomFilters`                                     | -                                                 | Кастомные фильтры                                    |
| [`onFiltersUpdated`]                                | `(customFilters: CustomSavedFilters<T>) => void`    | -                                                 | Колбэк на изменение кастомных фильтров               |
