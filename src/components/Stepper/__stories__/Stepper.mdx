...
Компонент Stepper позволяет пользователю работать с контентом пошагово.
...
import { Stepper } from '@consta/uikit/Stepper';
      ...
      case 0:
        return (
          <>
            ...
            <BasicSelect className={cnStepperStories('Select')} id='select' placeholder='Список' options={['1', 'вторая опция', '3', 'e']} getOptionLabel={o => o} />
          </>
        );
      case 1:
        return (
  ...
  return (
    <div className={cnStepperStories()}>
      ...
      <div className={cnStepperStories('Actions')}>
        ...
        <div>
          {(skippedSteps.size > 0 || completedSteps.size > 0) && <Button className={cnStepperStories('ResetButton')} onClick={handleReset} label="Отменить" view='ghost' />}
